...
public var player : GameObject;                        // Reference to the player.
...
public var resetPosition : Vector3;

var hitWithBullet : boolean;
var heardGunShot : boolean;
...
function OnTriggerStay (other : Collider)
{
    ...
    if(other.gameObject == player)
    {
        ...
        if(playerInSight || personalLastSighting != resetPosition)
        {
        	nav.enabled = true;
       ...
       // if(playerLayerZeroStateHash == hash.locomotionState || playerLayerOneStateHash == hash.shoutState)
        //{
        if(hitWithBullet || heardGunShot || playerInSight)
        {
...
function ChangeTurnSpeed()
{
	
	var speedMultiplyer : float = 1.0f - 0.9f*Vector3.Angle(transform.forward, nav.steeringTarget-transform.position)/180.0f;
	nav.speed = nav.speed*speedMultiplyer;
